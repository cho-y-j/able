...
| Backend tests | 885 tests (pytest) |
...
### Phase 21: Notification System Bugfix & Tests (`099ddf8`)

#### Critical Bug Fixes
- **`mark_read` / `mark_all_read` DB commit 누락**: 알림 읽음 처리가 DB에 저장되지 않던 치명적 버그 수정 — `await db.commit()` 추가
- **`update_preferences` DB commit 누락**: 알림 설정 변경이 영구 저장되지 않던 버그 수정
- **새 preferences 생성 시 기본값 미적용**: `NotificationPreference` 생성 시 명시적 defaults 설정

#### WebSocket Manager 개선
- `send_to_user` / `broadcast`에서 에러 무시하던 것을 로깅으로 변경
- Dead connection 자동 정리 로직 추가

#### P&L Alert Dedup 메모리 누수 수정
- `_last_pnl_alert` dict가 무한 증가하던 문제 해결
- 500개 초과 시 자동 pruning (절반 제거)

#### Notification Integration Tests (12 tests)
- List: 빈 목록, 데이터 있는 목록, 카테고리 필터
- Unread count: 0건, 5건
- Mark read: 성공, 404, 전체 읽음 — **db.commit() 호출 검증 포함**
- Preferences: 기본값 조회, 기존 설정 조회, 업데이트, 신규 생성
...
### Backend Tests (885 tests)
- Unit tests: signals, services, models, agents, API endpoints
- Integration tests: recipes API (9), market API (6), keys API (8), notifications API (12), E2E flows
...
```
099ddf8 Fix notification bugs: missing db.commit, WS error logging, P&L dedup leak
